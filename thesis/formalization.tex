...
Variables can be either bound or free. Expressions  $\lambda$, ``let'', the handler $\handler{a}{x}{\rho}{e}{\rho'}$ are binding operators for variables: the notions of free variables fv arise as expected.
...
We let $i$ range over pools of running \emph{instances}.%, i.e., the active part of a system which is spawned when a message is received by an handler. An instance is a running expression, which is granted a set of permissions. 
...
\section{Safety despite compromise}
...
\begin{comment}
\subsection{Message by the opponent.}
...
%For our running example of section \ref{sec:Example}, we are able to analyse the code with respect
...
In section \ref{sec:Example} we showed how an opponent that compromises cs1 was able to escalate only permission $\rho$ because $\permsleak{\rho}(\absC) = \rho$, while an opponent with permission \texttt{SEND} was able to escalate up to $\rho \join \rho'$ because $\permsleak{{SEND}}(\absC) = \rho \join \rho'$.
