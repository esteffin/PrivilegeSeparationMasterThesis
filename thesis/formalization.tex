...
Variables can be either bound or free. Expressions  $\lambda$, ``let'' the handler $\handler{a}{x}{\rho}{e}{\rho'}$ are binding operators for variables: the notions of free variables fv arise as expected. In other words the three construct over, bind a value to a variable and in the expression that follows the bind the variable lookup gives the value of its in the bind. \todo{stabile?}
...
b:   SEND ---> rho
\end{verbatim}
which captures the correct information for a CS1-opponent (i.e., a CS1-opponent can only escalate $\rho$). \todo {lasciato uguale...}
